#pragma once
#ifndef UTILS_SIGNCODE_H
#define UTILS_SIGNCODE_H
#include <string>
#include <Shlobj.h>
#include <functional>
...
public:
    using WarnHangle = void(*)(std::string);
    using ErrorHangle = void(*)(std::string);

    /**
     * @brief 注册警告和错误消息函数
     */
    void onWarn(WarnHangle handle) { _warnHandle = handle; };
    /**
     * @brief 注册警告和错误消息函数
     */
    void onError(ErrorHangle handle) { _errorHandle = handle; };

private:
    static WarnHangle _warnHandle;
    static ErrorHangle _errorHandle;
